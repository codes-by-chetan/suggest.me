...
const userSuggestionsSchema = new mongoose.Schema(
  {
    ...
    contentType: {
      ...
      enum: {
        ...
        message: "Content type must be one of: Movie, Series, Book, Music",
      },
    ...
    status: {
      type: String,
      enum: {
        values: ["WantToConsume", "Consuming", "Consumed", "NotInterested"],
        message:
...
userSuggestionsSchema.index({ sender: 1, recipient: 1, content: 1 }, { unique: true });
...
userSuggestionsSchema.pre("save", middlewares.dbLogger("Suggestions"));
...
const UserSuggestions = mongoose.model("UserSuggestions", userSuggestionsSchema);
