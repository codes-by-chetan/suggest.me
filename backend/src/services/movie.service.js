    ...
    if(!person){
        person = await models.Person.create({
            ...
            profileImage: personData.profile_path
              ? { url: `https://image.tmdb.org/t/p/w500${personData.profile_path}`, publicId: personData.profile_path }
              : null,
            isActive: true,
  ...
  try {
    ...
    company = await models.ProductionCompany.create({
      ...
      logo: companyData.logo_path
        ? { url: `https://image.tmdb.org/t/p/w500${companyData.logo_path}`, publicId: companyData.logo_path }
        : null,
  ...
  try {
    ...
    const tmdbMovieDetails = {
      ...
      poster: movie.poster_path
        ? { url: `https://image.tmdb.org/t/p/w500${movie.poster_path}`, publicId: movie.poster_path }
        : null,
      backdrop: movie.backdrop_path
        ? { url: `https://image.tmdb.org/t/p/w500${movie.backdrop_path}`, publicId: movie.backdrop_path }
        : null,
    ...
    console.log("TMDB Movie Details:", JSON.stringify(tmdbMovieDetails, null, 2));
...
const getMovieDetails = async (movieId, userId) => {
  ...
  try {
    ...
    throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, `Failed to save movie to database: ${error.message}`);
  }
};
