...
const Home = () => {
  const [isAuthenticated, _setIsAuthenticated] = useState(true); // Mock authentication state
  const [isSuggestionFlowOpen, setIsSuggestionFlowOpen] = useState(false);
  const [recentSuggestions, _setRecentSuggestions] =
    useState<ContentItem[]>(contentItemArray);
  const [suggestors, _setSuggestors] = useState<Suggestor[]>(suggestorsArray);
  const [_isAuthenticationOpen, setIsAuthenticationOpen] = useState(false);
  ...
  return (
    <main className='w-full px-4 pt-0 pb-[10vh]'>
      <div className='py-6'>
